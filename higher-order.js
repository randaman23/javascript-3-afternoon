...
//Code Here
let totalPopulation = populations.reduce((total, val)=> total + val) //  = populations.reduce(/* Provide Your Callback Here */)
...
let ordersTotal = orders.map((val)=> val * val.tax) //Code Here
...
*/
let bobsPurchases = purchases.filter((val)=> val.owner === 'Bob')
bobsPrices = []
for (let i=0;i<bobsPurchases.length;i++){
  bobsPrices.push(bobsPurchases[i].price)
}
let bobsTotal = bobsPrices.reduce((total, val)=> total + val)
